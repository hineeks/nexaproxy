...
object PluginManager {
    class PluginNotFoundException(val plugin: String) : FileNotFoundException(plugin),
        ...
        override fun getLocalizedMessage() = NexaProxy.application.getString(R.string.plugin_unknown, plugin)
    }
    ...
    private fun initNative(configuration: PluginConfiguration): InitResult? {
        ...
        val providers = NexaProxy.application.packageManager.queryIntentContentProviders(
        ...
        try {
            return initNativeFast(NexaProxy.application.contentResolver, options, uri)?.let { InitResult(it, options, isV2) }
        } catch (t: Throwable) {
        ...
        try {
            return initNativeSlow(NexaProxy.application.contentResolver, options, uri)?.let { InitResult(it, options, isV2) }
        } catch (t: Throwable) {
            failure?.also { t.addSuppressed(it) }
            throw t
        }
    ...
    fun ComponentInfo.loadString(key: String) = when (val value = metaData.get(key)) {
        is String -> value
        is Int -> NexaProxy.application.packageManager.getResourcesForApplication(applicationInfo).getString(value)
        null -> null
