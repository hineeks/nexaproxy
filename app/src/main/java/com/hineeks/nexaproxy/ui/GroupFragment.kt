...
import com.hineeks.nexaproxy.GroupType
import com.hineeks.nexaproxy.R
import com.hineeks.nexaproxy.NexaProxy
import com.hineeks.nexaproxy.database.DataStore
import com.hineeks.nexaproxy.database.GroupManager
import com.hineeks.nexaproxy.database.ProxyGroup
import com.hineeks.nexaproxy.database.NexaProxyDatabase
import com.hineeks.nexaproxy.databinding.LayoutGroupItemBinding
...
class GroupFragment : ToolbarFragment(R.layout.layout_group),
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_update_all_subscriptions -> {
                val connected = NexaProxy.started && DataStore.startedProfile > 0
                MaterialAlertDialogBuilder(requireContext())
                    .setTitle(R.string.update_all_subscriptions)
                    .setPositiveButton(android.R.string.ok) { _, _ ->
                        NexaProxyDatabase.groupDao.allGroups()
    ...
    private val exportProfiles = registerForActivityResult(ActivityResultContracts.CreateDocument()) { data ->
        if (data != null) {
            runOnDefaultDispatcher {
                val profiles = NexaProxyDatabase.proxyDao.getByGroup(selectedGroup.id)
    ...
    inner class GroupAdapter : RecyclerView.Adapter<GroupHolder>(),
        ...
        fun reload() {
            val groups = NexaProxyDatabase.groupDao.allGroups().toMutableList()
        ...
        fun commitMove() = runOnDefaultDispatcher {
            updated.forEach { NexaProxyDatabase.groupDao.updateGroup(it) }
            updated.clear()
        }
    ...
    inner class GroupHolder(binding: LayoutGroupItemBinding) : RecyclerView.ViewHolder(binding.root),
        ...
        override fun onMenuItemClick(item: MenuItem): Boolean {
            ...
            when (item.itemId) {
                ...
                R.id.action_subscription_link_clipboard -> {
                    ...
                    runOnDefaultDispatcher {
                        onMainDispatcher {
                            NexaProxy.trySetPrimaryClip(link)
                ...
                R.id.action_clipboard -> {
                    runOnDefaultDispatcher {
                        val profiles = NexaProxyDatabase.proxyDao.getByGroup(proxyGroup.id)
                        val links = profiles.mapNotNull {
                            try {
                                it.toLink()
                            } catch (_: Exception) {
                        ...
                        onMainDispatcher {
                            NexaProxy.trySetPrimaryClip(links)
                            snackbar(R.string.action_export_msg).show()
                        }
                    }
                }
        ...
        fun bind(group: ProxyGroup) {
            ...
            runOnDefaultDispatcher {
                val size = NexaProxyDatabase.proxyDao.countByGroup(group.id)
                onMainDispatcher {
                    try {
                        when (group.type) {
                            GroupType.BASIC -> {
