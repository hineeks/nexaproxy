...
import com.hineeks.nexaproxy.databinding.LayoutTrafficBinding
            ...
            when (item.itemId) {
                R.id.copy_label -> {
                    packageName?.let {
                        ...
                        val success = NexaProxy.trySetPrimaryClip(PackageCache.loadLabel(it))
                        snackbar(if (success) R.string.copy_success else R.string.copy_failed).show()
                    }
                }
                R.id.copy_package_name -> {
                    packageName?.let {
                        val success = NexaProxy.trySetPrimaryClip(it)
                        snackbar(if (success) R.string.copy_success else R.string.copy_failed).show()
                    }
                }
                R.id.create_rule -> {
