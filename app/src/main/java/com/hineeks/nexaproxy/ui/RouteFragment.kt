...
import com.hineeks.nexaproxy.database.RuleEntity
import com.hineeks.nexaproxy.database.NexaProxyDatabase
import com.hineeks.nexaproxy.databinding.LayoutEmptyRouteBinding
import com.hineeks.nexaproxy.databinding.LayoutRouteItemBinding
        ...
        when (item.itemId) {
            ...
            R.id.action_disable_all -> {
                runOnDefaultDispatcher {
                    NexaProxyDatabase.rulesDao.enableAll(enabled = false)
                    ruleAdapter.reload()
                    onMainDispatcher {
                        needReload()
                    }
            ...
            R.id.action_enable_all -> {
                runOnDefaultDispatcher {
                    NexaProxyDatabase.rulesDao.enableAll()
                    ruleAdapter.reload()
                    onMainDispatcher {
                        needReload()
                    }
            ...
            R.id.action_export -> {
                runOnDefaultDispatcher {
                    try {
                        val rules = NexaProxyDatabase.rulesDao.allRules()
                        if (rules.isNotEmpty()) {
                            ...
                            if (!jsonArray.isEmpty) {
                                onMainDispatcher {
                                    if (NexaProxy.trySetPrimaryClip(jsonArray.toString())) {
    ...
    inner class RuleAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>(), ProfileManager.RuleListener, UndoSnackbarManager.Interface<RuleEntity> {
        ...
        fun commitMove() = runOnDefaultDispatcher {
            if (updated.isNotEmpty()) {
                NexaProxyDatabase.rulesDao.updateRules(updated.toList())
                updated.clear()
                needReload()
            }
        }
