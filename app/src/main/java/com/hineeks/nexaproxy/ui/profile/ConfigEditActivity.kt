...
import com.hineeks.nexaproxy.databinding.LayoutEditConfigBinding
import com.hineeks.nexaproxy.ktx.getColorAttr
import com.hineeks.nexaproxy.ktx.onMainDispatcher
import com.hineeks.nexaproxy.ktx.runOnDefaultDispatcher
import com.hineeks.nexaproxy.ui.ThemedActivity
        ...
        override fun handleOnBackPressed() {
            if (ViewCompat.getRootWindowInsets(binding.editor)
                ...
                this@ConfigEditActivity.currentFocus?.windowToken?.let {
                    NexaProxy.ime.hideSoftInputFromWindow(it, 0)
                }
            } else {
    ...
    fun mkTheme(): ColorScheme {
        ...
        return ColorScheme(
            textColor = when (Theme.getTheme()) {
                R.style.Theme_NexaProxy_Black -> Color.WHITE
                R.style.Theme_NexaProxy_LightBlack -> Color.BLACK
                else -> colorPrimary
            },
            backgroundColor = when (Theme.getTheme()) {
                R.style.Theme_NexaProxy_Black -> Color.BLACK
                R.style.Theme_NexaProxy_LightBlack -> Color.WHITE
            ...
            selectionColor = when (Theme.getTheme()) {
                R.style.Theme_NexaProxy_Black -> "#4C4C4C".toColorInt()
                R.style.Theme_NexaProxy_LightBlack -> "#B3B3B3".toColorInt()
                else -> colorPrimary
            },
