...
import com.hineeks.nexaproxy.*
import com.hineeks.nexaproxy.aidl.TrafficStats
import com.hineeks.nexaproxy.bg.BaseService
import com.hineeks.nexaproxy.bg.test.V2RayTestInstance
import com.hineeks.nexaproxy.database.*
import com.hineeks.nexaproxy.databinding.LayoutProfileBinding
import com.hineeks.nexaproxy.databinding.LayoutProfileListBinding
import com.hineeks.nexaproxy.databinding.LayoutProgressListBinding
import com.hineeks.nexaproxy.fmt.AbstractBean
import com.hineeks.nexaproxy.fmt.exportBackup
import com.hineeks.nexaproxy.fmt.v2ray.StandardV2RayBean
import com.hineeks.nexaproxy.fmt.wireguard.toConf
import com.hineeks.nexaproxy.group.GroupUpdater
import com.hineeks.nexaproxy.group.Protocols
import com.hineeks.nexaproxy.group.RawUpdater
import com.hineeks.nexaproxy.ktx.*
import com.hineeks.nexaproxy.plugin.PluginManager
import com.hineeks.nexaproxy.ui.profile.*
import com.hineeks.nexaproxy.widget.QRCodeDialog
import com.hineeks.nexaproxy.widget.UndoSnackbarManager
...
import com.hineeks.nexaproxy.utils.FormatFileSizeCompat
...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        toolbar.setOnLongClickListener {
            val selectedProxy = selectedItem
                ?: NexaProxyDatabase.proxyDao.getById(DataStore.selectedProxy)
    ...
    suspend fun import(proxies: List<AbstractBean>) {
        if (proxies.size <= 32) {
            ...
            val name = NexaProxyDatabase.groupDao.getById(DataStore.selectedGroupForImport())!!.displayName()
            onMainDispatcher {
    ...
    private suspend fun finishImport(proxies: List<AbstractBean>) {
        ...
        onMainDispatcher {
            ...
            val group = NexaProxyDatabase.groupDao.getById(targetId)!!
    ...
    override fun onMenuItemClick(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_import_clipboard -> {
                val text = NexaProxy.getClipboardText()
                if (text.isBlank()) {
            ...
            R.id.action_clear_traffic_statistics -> {
                runOnDefaultDispatcher {
                    val profiles = NexaProxyDatabase.proxyDao.getByGroup(DataStore.currentGroupId())
    ...
    private fun ProxyEntity.useBrowserForwarder(): Boolean {
        return when (val bean = requireBean()) {
            ...
            is ChainBean -> {
                NexaProxyDatabase.proxyDao.getEntities(bean.proxies).any {
    ...
    fun urlTest() {
        ...
        val mainJob = runOnDefaultDispatcher {
            ...
            var profilesUnfiltered = NexaProxyDatabase.proxyDao.getByGroup(group.id)
            ...
            repeat(6) {
                testJobs.add(launch {
                    while (isActive) {
                        ...
                        try {
                            val instance = if (DataStore.tunImplementation == TunImplementation.SYSTEM && DataStore.serviceMode == Key.MODE_VPN && NexaProxy.started && DataStore.startedProfile > 0) {
                                V2RayTestInstance(profile, link, timeout, protectPath = NexaProxy.deviceStorage.noBackupFilesDir.toString() + "/protect_path")
                            } else {
    ...
    inner class GroupPagerAdapter : FragmentStateAdapter(this),
        ...
        fun reload(now: Boolean = false) {
            ...
            runOnDefaultDispatcher {
                var newGroupList = ArrayList(NexaProxyDatabase.groupDao.allGroups())
                if (newGroupList.isEmpty()) {
                    NexaProxyDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true))
                    newGroupList = ArrayList(NexaProxyDatabase.groupDao.allGroups())
                }
                newGroupList.find { it.ungrouped }?.let {
                    if (NexaProxyDatabase.proxyDao.countByGroup(it.id) == 0L) {
        ...
        override suspend fun onRemoved(groupId: Long, profileId: Long) {
            ...
            if (group.ungrouped && NexaProxyDatabase.proxyDao.countByGroup(groupId) == 0L) {
                reload()
            }
        }
    }
    ...
    class GroupFragment : Fragment() {
        ...
        inner class ConfigurationAdapter : RecyclerView.Adapter<ConfigurationHolder>(),
            ...
            override fun onBindViewHolder(
                ...
                if (payloads.contains("PAYLOAD_SELECTION_CHANGE")) {
                    ...
                    val isStarted = isSelected && NexaProxy.started && DataStore.startedProfile == entityId
            ...
            fun commitMove() = runOnDefaultDispatcher {
                updated.forEach { NexaProxyDatabase.proxyDao.updateProxy(it) }
                updated.clear()
            }
            ...
            override suspend fun groupUpdated(groupId: Long) {
                if (groupId != proxyGroup.id) return
                proxyGroup = NexaProxyDatabase.groupDao.getById(groupId)!!
                reloadProfiles()
            }
            ...
            fun reloadProfiles() {
                ...
                var newProfiles = NexaProxyDatabase.proxyDao.getByGroup(proxyGroup.id)
        ...
        inner class ConfigurationHolder(val view: View) : RecyclerView.ViewHolder(view),
            ...
            fun bind(proxyEntity: ProxyEntity) {
                ...
                if (parent.select) {
                    ...
                    view.setOnClickListener {
                        runOnDefaultDispatcher {
                            ...
                            } else if (NexaProxy.isTv) {
                                if (NexaProxy.started) {
                ...
                runOnDefaultDispatcher {
                    val selected = (parent.selectedItem?.id
                        ?: DataStore.selectedProxy) == proxyEntity.id
                    val started = selected && NexaProxy.started && DataStore.startedProfile == proxyEntity.id
                    onMainDispatcher {
            ...
            fun export(link: String) {
                val success = NexaProxy.trySetPrimaryClip(link)
