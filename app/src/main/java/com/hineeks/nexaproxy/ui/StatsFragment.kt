...
import com.hineeks.nexaproxy.databinding.LayoutTrafficItemBinding
import com.hineeks.nexaproxy.databinding.LayoutTrafficListBinding
...
class StatsFragment : Fragment(R.layout.layout_traffic_list) {
    ...
    fun emitStats(statsList: List<AppStats>) {
        ...
        for (stats in NexaProxyDatabase.statsDao.all()) {
        ...
        if (data.isEmpty()) {
            runOnMainDispatcher {
                ...
                if (!NexaProxy.started || DataStore.serviceMode != Key.MODE_VPN) {
