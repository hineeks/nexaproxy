/******************************************************************************
 * Copyright (C) 2026 by hineeks <contact-git@sekai.icu>                  *
 *                                                                            *
...
import com.hineeks.nexaproxy.BuildConfig
import com.hineeks.nexaproxy.R
import com.hineeks.nexaproxy.NexaProxy
import com.hineeks.nexaproxy.database.*
import com.hineeks.nexaproxy.database.preference.KeyValuePair
import com.hineeks.nexaproxy.database.preference.PublicDatabase
import com.hineeks.nexaproxy.databinding.LayoutBackupBinding
import com.hineeks.nexaproxy.databinding.LayoutImportBinding
import com.hineeks.nexaproxy.databinding.LayoutProgressBinding
...
class BackupFragment : NamedFragment(R.layout.layout_backup) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.actionExport.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                onMainDispatcher {
                    startFilesForResult(
                        exportSettings, "nexaproxy_backup_${System.currentTimeMillis()}.json"
                    )
                }
            }
        }
        ...
        binding.actionShare.setOnClickListener {
            runOnDefaultDispatcher {
                ...
                app.cacheDir.mkdirs()
                val cacheFile = File(
                    app.cacheDir, "nexaproxy_backup_${System.currentTimeMillis()}.json"
                )
    ...
    fun doBackup(profile: Boolean, rule: Boolean, setting: Boolean): String {
        ...
        if (profile) {
            out.add("profiles", JsonArray().apply {
                NexaProxyDatabase.proxyDao.getAll().forEach {
                    add(it.toBase64Str())
                }
            })

            out.add("groups", JsonArray().apply {
                NexaProxyDatabase.groupDao.allGroups().forEach {
                    add(it.toBase64Str())
                }
            })
        }
        if (rule) {
            out.add("rules", JsonArray().apply {
                NexaProxyDatabase.rulesDao.allRules().forEach {
                    add(it.toBase64Str())
                }
            })
            out.add("assets", JsonArray().apply {
                NexaProxyDatabase.assetDao.getAll().forEach {
                    add(it.toBase64Str())
                }
            })
        }
    ...
    fun finishImport(
        ...
        if (profile && content.contains("profiles")) {
            ...
            NexaProxyDatabase.proxyDao.reset()
            NexaProxyDatabase.proxyDao.insert(profiles)
            ...
            NexaProxyDatabase.groupDao.reset()
            NexaProxyDatabase.groupDao.insert(groups)
        }
        if (rule && content.contains("rules")) {
            ...
            NexaProxyDatabase.rulesDao.reset()
            NexaProxyDatabase.rulesDao.insert(rules)
            ...
            NexaProxyDatabase.assetDao.reset()
            NexaProxyDatabase.assetDao.insert(assets)
        }
