...
import com.hineeks.nexaproxy.ktx.listenForPackageChanges
import com.hineeks.nexaproxy.plugin.PluginContract.METADATA_KEY_ID
import java.io.File
...
object PluginManager {
    ...
    class PluginNotFoundException(val plugin: String) : FileNotFoundException(plugin),
        ...
        override fun getLocalizedMessage() =
            NexaProxy.application.getString(R.string.plugin_unknown, plugin)
    }

    private var receiver: BroadcastReceiver? = null
    private var cachedPlugins: PluginList? = null
    fun fetchPlugins() = synchronized(this) {
        if (receiver == null) receiver = NexaProxy.application.listenForPackageChanges {
            synchronized(this) {
                receiver = null
                cachedPlugins = null
            }
    ...
    private fun initNative(pluginId: String): InitResult? {
        ...
        var providers = NexaProxy.application.packageManager.queryIntentContentProviders(
            Intent(PluginContract.ACTION_NATIVE_PLUGIN, buildUri(pluginId, "com.hineeks.nexaproxy")), flags)
        ...
        if (providers.isEmpty()) {
            providers = NexaProxy.application.packageManager.queryIntentContentProviders(
        ...
        if (providers.size > 1) {
            ...
            Toast.makeText(NexaProxy.application, message, Toast.LENGTH_LONG).show()
        ...
        try {
            return initNativeFast(NexaProxy.application.contentResolver,
                pluginId,
        ...
        try {
            return initNativeSlow(NexaProxy.application.contentResolver,
                pluginId,
                uri)?.let { InitResult(it) }
        } catch (t: Throwable) {
            failure.also { t.addSuppressed(it) }
    ...
    private fun initNativeInternal(pluginId: String): String? {
        return when (pluginId) {
            ...
            File(NexaProxy.application.applicationInfo.nativeLibraryDir, it).apply {
                check(canExecute())
            }.absolutePath
        }
    }
    ...
    private fun initNativeSlow(cr: ContentResolver, pluginId: String, uri: Uri): String? {
        ...
        val pluginDir = File(NexaProxy.deviceStorage.noBackupFilesDir, "plugin")
        (cr.query(uri,
    ...
    fun ComponentInfo.loadString(key: String) = when (val value = metaData.get(key)) {
        is String -> value
        is Int -> NexaProxy.application.packageManager.getResourcesForApplication(applicationInfo)
            .getString(value)
        null -> null
