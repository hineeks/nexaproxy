...
import com.hineeks.nexaproxy.NexaProxy.Companion.application
import com.hineeks.nexaproxy.bg.VpnService
import com.hineeks.nexaproxy.database.preference.InMemoryDatabase
...
import com.hineeks.nexaproxy.database.preference.RoomPreferenceDataStore
...
object DataStore : OnPreferenceDataStoreChangeListener {
    ...
    var selectedGroup by configurationStore.long(Key.PROFILE_GROUP) {
        NexaProxy.currentProfile?.groupId ?: 0L
    }

    fun currentGroupId(): Long {
        val currentSelected = selectedGroup
        if (currentSelected > 0L) return currentSelected
        val groups = NexaProxyDatabase.groupDao.allGroups()
        if (groups.isNotEmpty()) {
            val groupId = groups[0].id
            selectedGroup = groupId
            return groupId
        }
        val groupId = NexaProxyDatabase.groupDao.createGroup(ProxyGroup(ungrouped = true))
        selectedGroup = groupId
        return groupId
    }
    ...
    fun currentGroup(): ProxyGroup {
        ...
        if (currentSelected > 0L) {
            group = NexaProxyDatabase.groupDao.getById(currentSelected)
        }
        if (group != null) return group
        val groups = NexaProxyDatabase.groupDao.allGroups()
        if (groups.isEmpty()) {
            group = ProxyGroup(ungrouped = true).apply {
                id = NexaProxyDatabase.groupDao.createGroup(this)
            }
        } else {
            group = groups[0]
        }
