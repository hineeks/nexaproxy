...
object GroupManager {
    ...
    suspend fun clearGroup(groupId: Long) {
        if (NexaProxyDatabase.proxyDao.isIdInGroup(DataStore.selectedProxy, groupId) > 0) {
            DataStore.selectedProxy = 0L
        }
        NexaProxyDatabase.proxyDao.deleteAll(groupId)
        iterator { groupUpdated(groupId) }
    }

    fun rearrange(groupId: Long) {
        val entities = NexaProxyDatabase.proxyDao.getByGroup(groupId)
        for (index in entities.indices) {
            entities[index].userOrder = (index + 1).toLong()
        }
        NexaProxyDatabase.proxyDao.updateProxy(entities)
    }
    ...
    suspend fun postUpdate(groupId: Long) {
        postUpdate(NexaProxyDatabase.groupDao.getById(groupId) ?: return)
    }
    ...
    suspend fun createGroup(group: ProxyGroup): ProxyGroup {
        group.userOrder = NexaProxyDatabase.groupDao.nextOrder() ?: 1
        group.id = NexaProxyDatabase.groupDao.createGroup(group.applyDefaultValues())
    ...
    suspend fun updateGroup(group: ProxyGroup, reconfigureUpdater: Boolean = true) {
        NexaProxyDatabase.groupDao.updateGroup(group)
        iterator { groupUpdated(group) }
    ...
    suspend fun deleteGroup(groupId: Long) {
        val group = NexaProxyDatabase.groupDao.getById(groupId)
        NexaProxyDatabase.groupDao.deleteById(groupId)
        NexaProxyDatabase.proxyDao.deleteByGroup(groupId)
    ...
    suspend fun deleteGroup(group: List<ProxyGroup>) {
        NexaProxyDatabase.groupDao.deleteGroup(group)
        NexaProxyDatabase.proxyDao.deleteByGroup(group.map { it.id }.toLongArray())
