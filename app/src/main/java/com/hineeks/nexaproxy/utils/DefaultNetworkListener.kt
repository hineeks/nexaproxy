...
object DefaultNetworkListener {
    ...
    suspend fun get() = if (fallback) @TargetApi(23) {
        NexaProxy.connectivity.activeNetwork
        ...
        try {
            ...
            when (Build.VERSION.SDK_INT) {
                in 31..Int.MAX_VALUE -> @TargetApi(31) {
                    NexaProxy.connectivity.registerBestMatchingNetworkCallback(
                        request, Callback, mainHandler
                    )
                }
                in 28 until 31 -> @TargetApi(28) {  // we want REQUEST here instead of LISTEN
                    NexaProxy.connectivity.requestNetwork(request, Callback, mainHandler)
                }
                in 26 until 28 -> @TargetApi(26) {
                    NexaProxy.connectivity.registerDefaultNetworkCallback(Callback, mainHandler)
                }
                in 24 until 26 -> @TargetApi(24) {
                    NexaProxy.connectivity.registerDefaultNetworkCallback(Callback)
                }
                else -> {
                    NexaProxy.connectivity.requestNetwork(request, Callback)
    ...
    private fun unregister() = NexaProxy.connectivity.unregisterNetworkCallback(Callback)
}
