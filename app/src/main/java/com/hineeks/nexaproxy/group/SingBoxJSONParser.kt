...
package com.hineeks.nexaproxy.group
...
import com.hineeks.nexaproxy.fmt.shadowsocks.supportedShadowsocksMethod
import com.hineeks.nexaproxy.fmt.shadowsocksr.ShadowsocksRBean
import com.hineeks.nexaproxy.fmt.shadowsocksr.supportedShadowsocksRMethod
import com.hineeks.nexaproxy.fmt.shadowsocksr.supportedShadowsocksRObfs
import com.hineeks.nexaproxy.fmt.shadowsocksr.supportedShadowsocksRProtocol
...
import com.hineeks.nexaproxy.fmt.tuic5.supportedTuic5CongestionControl
import com.hineeks.nexaproxy.fmt.tuic5.supportedTuic5RelayMode
import com.hineeks.nexaproxy.fmt.v2ray.VLESSBean
import com.hineeks.nexaproxy.fmt.v2ray.VMessBean
import com.hineeks.nexaproxy.fmt.v2ray.supportedVmessMethod
                        ...
                        (tls.getBoolean("enabled"))?.also { enabled ->
                            if (enabled) {
                                ...
                                if (v2rayBean.alpn == null && v2rayBean.type == "quic") {
                                    // https://github.com/NexaProxy/sing-box/pull/1934
                                    v2rayBean.alpn = "h3"
                                }
            ...
            when (type) {
                ...
                "vmess" -> {
                    ...
                    outbound.getString("uuid").orEmpty().also {
                        // see https://github.com/NexaProxy/sing-vmess/blob/31ec11e8790c48a4bf11711806a61e929ee6b89d/client.go#L37-L40
                ...
                "vless" -> {
                    ...
                    outbound.getString("uuid").orEmpty().also {
                        // see https://github.com/NexaProxy/sing-vmess/blob/31ec11e8790c48a4bf11711806a61e929ee6b89d/vless/client.go#L28-L31
                        v2rayBean.uuid = parseUUID(it)?.toHexDashString() ?: uuid5(it)
                    }
                    v2rayBean.packetEncoding = when (outbound.getString("packet_encoding")) {
                        "packetaddr" -> "packet"
        ...
        "ssh" -> {
            val sshBean = SSHBean().apply {
                ...
                outbound.getInt("server_port")?.takeIf { it > 0 }?.also {
                    ...
                    // https://github.com/NexaProxy/sing-box/blob/5f739c4e66163ce88dbf75456e698c191d7069a6/protocol/ssh/outbound.go#L68
                    serverPort = 22
                }
                outbound.getString("user")?.takeIf { it.isNotEmpty() }?.also {
                    username = it
                } ?: {
                    // https://github.com/NexaProxy/sing-box/blob/5f739c4e66163ce88dbf75456e698c191d7069a6/protocol/ssh/outbound.go#L71
                    username = "root"
                }
