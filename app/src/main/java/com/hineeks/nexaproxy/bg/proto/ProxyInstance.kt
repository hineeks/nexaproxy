    ...
    fun sendInitStatuses() {
        ...
        for (observatoryTag in config.observatoryTags) {
            ...
            val profile = when {
                ...
                else -> NexaProxyDatabase.proxyDao.getById(id)
            } ?: continue
        ...
        if (id != null) {
            val profile = when {
                ...
                statsOutbounds.containsKey(id) -> statsOutbounds[id]!!.proxyEntity
                    ...
                    NexaProxyDatabase.proxyDao.getById(id)
                }
            }
        ...
        runBlocking {
            try {
                ...
                if (outboundStats.uplinkTotal + outboundStats.downlinkTotal != 0L) {
                    NexaProxyDatabase.proxyDao.updateTraffic(
                        profile.id,
