...
import com.hineeks.nexaproxy.database.StatsEntity
import com.hineeks.nexaproxy.fmt.LOCALHOST
...
import com.hineeks.nexaproxy.ui.VpnRequestActivity
import com.hineeks.nexaproxy.utils.DefaultNetworkListener
import com.hineeks.nexaproxy.utils.Subnet
...
class VpnService : BaseVpnService(),
    ...
    override fun acquireWakeLock() {
        wakeLock = NexaProxy.power.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "nexaproxy:vpn")
            .apply { acquire() }
    }
    ...
    override val tag = "NexaProxyVpnService"
    ...
    private fun startVpn() {
        ...
        val builder = Builder().setConfigureIntent(NexaProxy.configureIntent(this))
        ...
        if (tunImplementation == TunImplementation.SYSTEM) {
            config.protectPath = NexaProxy.deviceStorage.noBackupFilesDir.toString() + "/protect_path"
        }
    ...
    fun persistAppStats() {
        ...
        val all = NexaProxyDatabase.statsDao.all().associateBy { it.uid }
        for (stats in appStats) {
            val uid = stats.uid
            if (!all.containsKey(uid)) {
                NexaProxyDatabase.statsDao.create(
                    StatsEntity(
                        uid = uid,
        ...
        if (toUpdate.isNotEmpty()) {
            NexaProxyDatabase.statsDao.update(toUpdate)
        }
    }
